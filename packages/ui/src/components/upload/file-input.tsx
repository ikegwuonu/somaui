  ...
  variants: {
    ...
    size: {
      sm: {
        ...
        button:
          '[&::file-selector-button]:h-7 [&::file-selector-button]:px-2.5',
      },
      md: {
        container: 'pr-3.5 py-2 text-sm h-10 pl-[1px]',
        button:
          '[&::file-selector-button]:h-9 [&::file-selector-button]:px-3.5',
      },
      lg: {
...
export function FileInput({
  ...
  return (
    <div className={cn('somaui-file-input-root', 'flex flex-col', className)}>
      <label className="block">
        {label ? (
          <span
            className={cn(
              'somaui-file-input-label',
              'block',
      ...
      {error ? (
        <FieldErrorText
          size={size}
          error={error}
          className={cn('somaui-file-input-error-text', errorClassName)}
        />
      ) : null}
    </div>
  );
