...
const switchTV = tv({
  slots: {
    ...
    knob: "somaui-switch-knob relative flex justify-center items-center transform ring-0 transition duration-200 ease-in-out translate-x-[2.5px] rtl:-translate-x-[2.5px] bg-primary rounded-full",
    input:
      "peer/switch absolute -z-[1] opacity-0 [&:checked:enabled~span_.somaui-switch-off-icon]:hidden [&:checked:enabled~span_.somaui-switch-on-icon]:opacity-100 [&:checked:enabled~span>.somaui-switch-knob]:bg-white [&:checked:enabled~span>.somaui-switch-knob]:text-black [&:checked:enabled~span>.somaui-switch-knob]:dark:bg-black [&:checked:enabled~span>.somaui-switch-knob]:dark:text-white",
    icon: "",
    label: "somaui-switch-label",
  },
  variants: {
    variant: {
      flat: {
    ...
    size: {
      sm: {
        ...
        label: "text-xs",
        input:
          "[&:checked+span>.somaui-switch-knob]:translate-x-[.88rem] rtl:[&:checked+span>.somaui-switch-knob]:-translate-x-[.88rem]",
      },
      md: {
        container: "w-10 h-6",
        ...
        label: "text-sm",
        input:
          "[&:checked+span>.somaui-switch-knob]:translate-x-[1.14rem] rtl:[&:checked+span>.somaui-switch-knob]:-translate-x-[1.14rem]",
      },
      lg: {
        container: "w-12 h-7",
        ...
        label: "text-sm",
        input:
          "[&:checked+span>.somaui-switch-knob]:translate-x-[1.36rem] rtl:[&:checked+span>.somaui-switch-knob]:-translate-x-[1.36rem]",
      },
    },
    labelPlacement: {
      left: { label: "" },
  ...
  compoundVariants: [
    ...
    },
    {
      ...
      disabled: false,
      class:
        "[&:hover_.somaui-switch-knob]:bg-primary [&:hover_.somaui-switch-knob]:text-primary-foreground",
    },
    {
      variant: "outline",
      disabled: true,
      class:
        "[&:hover_.somaui-switch-knob]:!bg-muted [&:hover_.somaui-switch-knob]:!text-muted-foreground",
    },
    // Label margins
...
export function Switch({
  ...
  return (
    <div className={cn("somaui-switch", className)}>
      <label
        className={cn(
      ...
      >
        ...
        <span className={container({ className: switchClassName })}>
          ...
          >
            <span className="somaui-switch-off-icon inline-flex items-center">
              {offIcon}
            </span>
            <span className="somaui-switch-on-icon absolute inset-0 inline-flex items-center justify-center opacity-0">
              {onIcon || (
                // HeroIcon: check
                <svg
      ...
      {error ? (
        <FieldErrorText
          size={size}
          error={error}
          className={cn("somaui-switch-error-text", errorClassName)}
        />
      ) : null}
    </div>
  );
