...
A powerful and flexible date picker component built on top of **react-datepicker**. Seamlessly integrated with SomaUI's design system, this component provides a consistent and accessible way to select dates, times, and date ranges in your application.
...
- 🎨 **Theme Integration** - Fully integrated with SomaUI theme colors and design tokens
- 🌓 **Dark Mode Support** - Automatic dark mode styling using SomaUI theme variables
...
import { Input, InputProps } from '@somaui/ui/input';
import { cn } from 'somaui';
...
/* Uses SomaUI theme colors and design tokens */
...
.react-datepicker .react-datepicker__day--selected {
  @apply rounded-[var(--border-radius)] border-none bg-[var(--primary)] leading-7 font-normal text-[var(--primary-foreground)];
...
.react-datepicker .react-datepicker__month-read-view--down-arrow {
  @apply top-[5px] h-[7px] w-[7px] border-t-[length:var(--border-width)] border-r-[length:var(--border-width)] border-[var(--muted)];
}
