...
const SomaUIThemeContext = createContext<SomaUIThemeContextType | undefined>(
  undefined
);
...
export function SomaUIThemeProvider({
  ...
  const pendingUpdatesRef = useRef<
    Map<string, { value: string; theme: "light" | "dark" }>
  >(new Map());
  ...
  useEffect(() => {
    ...
    if (!lightStyle) {
      ...
      lightStyle.id = "somaui-light-theme-vars";
      document.head.appendChild(lightStyle);
    }

    if (!darkStyle) {
      darkStyle = document.createElement("style");
      darkStyle.id = "somaui-dark-theme-vars";
        ...
        setLightTokens((prev) => {
          ...
          if (updated[tokenName]) {
            updated[tokenName] = {
              ...
              oklch: value,
              hex:
                isOklch && updated[tokenName].hex
                  ? updated[tokenName].hex
                  : updated[tokenName].hex,
            };
          }
          return updated;
  ...
  return (
    <SomaUIThemeContext.Provider value={contextValue}>
      {children}
