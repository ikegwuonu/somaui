...
export default function RangeSlider({
  ...
  const value =
    props.value ??
    props.defaultValue ??
    (props.range ? [props.min ?? 0, props.max ?? 100] : (props.min ?? 0));
  ...
  return (
    <div>
      ...
      <div className="mt-5 flex items-center justify-between gap-5 text-sm font-bold">
        <div className="w-full max-w-[200px] overflow-hidden rounded-lg border border-gray-200 py-2">
          <label
            htmlFor="range-slider-min"
            className="block px-3 pt-1 text-gray-400"
          >
            Min
          </label>
          <input
            ...
            className="!bg-gray-lightest w-full border-none p-3 pb-1 text-sm outline-none focus:shadow-none focus:ring-0"
            min={0}
            max={state.max}
            readOnly
